...
const models: Model[] = [
  { id: "rolls-battery-s-550", manufacturer: "rolls-battery", name: "S-550", aliases: ["S550", "8 CS 27P"], dialects: [] },
  { id: "epever-xtra4210n", manufacturer: "epever", name: "XTRA4210N", aliases: [], dialects: [] },
];
const base = { models, source: "rolls-renewable-pdf", extractedBy: "ai:@cf/test@p1", confidence: "vendor-doc" as const };

test("a figure is attached only when the document's own name answers to a model we hold", () => {
  const { specs, unmatched } = specsFrom({ ...base, manufacturer: "rolls-battery", reports: [
    { model: "S-550", specs: [{ name: "Rated capacity", value: "428", unit: "Ah", conditions: "20-hour rate" }] },
...
test("one capacity at two discharge rates is two rows, which is why specs are not columns", () => {
  const { specs } = specsFrom({ ...base, manufacturer: "rolls-battery", reports: [{ model: "S-550", specs: [
...
test("a name matches through punctuation and case but never across a different maker", () => {
  ...
  assert.equal(matchModel(models, "epever", "S-550"), undefined, "a Rolls name must not reach an EPEver model");
});
...
test("a figure with no name or no value is dropped rather than stored empty", () => {
  const { specs } = specsFrom({ ...base, manufacturer: "epever", reports: [{ model: "XTRA4210N", specs: [
    { name: "", value: "40", unit: "A" }, { name: "Rated current", value: "  " }, { name: "Rated current", value: "40", unit: "A" },
  ] }] });
  assert.equal(specs.length, 1);
...
test("every extracted figure names the model that read it, so nothing looks confirmed", () => {
  const { specs } = specsFrom({ ...base, manufacturer: "epever", reports: [{ model: "XTRA4210N", specs: [{ name: "Rated current", value: "40", unit: "A" }] }] });
...
test("ids are stable, so a second extraction rewrites a figure rather than piling up duplicates", () => {
  ...
  assert.notEqual(specId("m", "Rated capacity", "20-hour rate"), specId("m", "Rated capacity", "100-hour rate"));
});
