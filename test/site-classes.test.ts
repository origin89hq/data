...
// The plate — the clipped control, the content column, the frame — and the site chrome come from
// @origin89/brand rather than being copied into the design folder, so the package's sheets are
// part of what the design defines. Reading them here also means this test fails if the package
// stops providing one.
...
const PACKAGED = ["tokens/plate.css", "tokens/chrome.css"].map((sheet) =>
  fromSite.resolve(`@origin89/brand/${sheet}`),
);
const stylesheet = [
  ...readdirSync(DESIGN)
    .filter((file) => file.endsWith(".css"))
    .map((file) => readFileSync(join(DESIGN, file), "utf8")),
  ...PACKAGED.map((sheet) => readFileSync(sheet, "utf8")),
].join("\n");
...
test("the check would notice a class that does not exist", () => {
  ...
  assert.equal(defined.has("o89-footer-grid"), true, "and one from its chrome sheet");
  assert.equal(
