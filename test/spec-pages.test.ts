...
const candidate = (url: string, figures: number, withUnit: number, models: string[]) => ({ url, products: models.length, figures, withUnit, models });
...
test("a buying guide is refused however many figures it carries, because it is somebody writing about equipment rather than the maker stating it", () => {
  ...
  const guide = candidate("https://www.bluetti.com/blogs/buying-guide/indoor-generators", 96, 96, ["AC180", "Elite 200 V2", "AC300"]);
  assert.match(refuses(guide, bar) ?? "", /article/);
  const post = candidate("https://sigineer.com/2016/07/07/5-dip-switches-functionality/", 20, 6, ["TPH 6KW"]);
...
test("a table headed by an attribute or a measurement is refused, since its figures are filed under the wrong thing", () => {
  const category = candidate("https://sigineer.com/product-category/inverter-chargers/three-phase-inverter-charger/", 35, 14, ["Rated Power", "DC Input", "AC Output 1"]);
  ...
  const matrix = candidate("https://no.co/support/nlp9", 28, 18, ["2 Amp-Hours", "25.6 Watt-Hours", "GENIUS1", "GENIUS5"]);
  assert.match(refuses(matrix, bar) ?? "", /2 Amp-Hours/);
  const switches = candidate("https://sigineer.com/dip/", 20, 6, ["Switch Function", "Position: 0"]);
  assert.match(refuses(switches, bar) ?? "", /Switch Function/);
});

test("a maker's own product table is adopted", () => {
  const tph = candidate("https://sigineer.com/product/24kw-48v-to-120-208v-3-phase-inverter-charger-pure-sine-wave/", 208, 58, ["TPH 6KW", "TPH 18KW", "TPH 36KW", "TPH 45KW"]);
  assert.equal(refuses(tph, bar), undefined);
  const genius = candidate("https://no.co/support/genius1", 24, 23, ["GENIUS1", "GENIUS2*", "GENIUS5", "GENIUS10"]);
...
test("a page too thin to be worth fetching is refused, and says which bar it missed", () => {
  assert.match(refuses(candidate("https://rollsbattery.com/battery/s12-90gel/", 3, 3, ["S12-90GEL"]), bar) ?? "", /only 3 figures/);
  assert.match(refuses(candidate("https://sigineer.com/product/avr/", 66, 0, ["100", "110"]), bar) ?? "", /carry a unit/);
});
