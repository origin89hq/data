...
test("nothing in the records claims to have happened after today", () => {
  assert.deepEqual(validate(base).errors.filter((e) => e.includes("has not happened")), []);
});

test("a source retrieved tomorrow is refused, because a run's label is not a date", () => {
  const records = { ...base, sources: [...base.sources, { id: "future", url: "https://x.test/a.pdf", retrievedAt: tomorrow }] };
...
test("a decision dated tomorrow is refused too, whichever record carries it", () => {
  ...
  const records = { ...base, brands: [{ ...brand, id: "future-brand", brand: "Future", checkedAt: tomorrow }] };
  assert.match(validate(records).errors.join("\n"), /decided on .*which has not happened/);
});

test("today is fine, so the check does not merely refuse everything", () => {
  const records = { ...base, sources: [...base.sources, { id: "now", url: "https://x.test/b.pdf", retrievedAt: today }] };
  assert.deepEqual(validate(records).errors.filter((e) => e.includes("has not happened")), []);
});
