...
import { declaredLanguage, withoutTranslations } from "@origin89/equipment-schema/documents";
...
test("a file name that marks a translation declares its language", () => {
  assert.equal(
    declaredLanguage("https://sol-ark.com/x/8K-2P-N_UserManual_v1.0_ES_05042025-1-1.pdf"),
    "es",
  );
...
test("an English or unmarked document declares nothing, and a bilingual one is not a translation", () => {
  assert.equal(
    declaredLanguage("https://x.com/SK140-0005-002-8K-2P-N-EN-Manual-1-1.pdf"),
    undefined,
  );
...
test("a translation is dropped only when the maker publishes something else", () => {
  const both = withoutTranslations([
    { url: "https://x.com/manual-ES.pdf" },
    { url: "https://x.com/manual-EN.pdf" },
  ]);
  assert.deepEqual(
    both.keep.map((d) => d.url),
    ["https://x.com/manual-EN.pdf"],
  );
...
test("no document already cited is dropped by a rule that cannot tell a language from a product name", () => {
  ...
  const surprising = loadRecords()
    ...
    .map((s) => {
      assert.ok(s.url);
      return `${declaredLanguage(s.url)}: ${s.url}`;
    });
  assert.ok(
    surprising.length <= 15,
    `more documents read as translations than the 15 seen: ${surprising.slice(0, 5).join(", ")}`,
  );
});
...
test("a maker's translated manual is spotted from what it said, not from what it is called", () => {
  ...
  const spanish = {
    url: "https://x.com/44514-s-manual.pdf",
    names: ["Corriente máxima", "Tensión de entrada", "Presión mínima", "Voltaje"],
  };
  const english = {
    url: "https://x.com/44514-manual.pdf",
    names: ["Maximum current", "Input voltage", "Minimum pressure", "Voltage"],
  };
  ...
  const only = [
    {
      ...
      names: ["Corriente máxima", "Tensión de entrada", "Presión mínima", "Tipo de batería"],
    },
    {
...
test("a document too short to judge never decides what language its maker publishes in", () => {
  ...
  assert.equal(
    keep.length,
    2,
    "one accented name in a one-line document is not a translated edition",
  );
});
