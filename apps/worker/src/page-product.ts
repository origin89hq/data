...
function offerOf(product: Record<string, unknown>): {
  price?: string;
  currency?: string;
  availability?: boolean;
} {
  ...
  const availability =
    typeof o.availability === "string"
      ? /InStock|LimitedAvailability|PreOrder/i.test(o.availability)
      : undefined;
  return {
  ...
  for (let m = OPEN_TAG.exec(scope); m; m = OPEN_TAG.exec(scope)) {
    ...
    if (nested) {
      ...
      if (prop && inner !== undefined) {
        const innerName = /<\w+[^>]*\bitemprop\s*=\s*["']name["'][^>]*>([\s\S]*?)<\/\w+>/i.exec(
          inner,
        );
    ...
    if (!prop) continue;
    const content =
      attr(attrs, "content") ?? (prop === "availability" ? attr(attrs, "href") : undefined);
...
export function extractProduct(
  ...
  | { product: PageProduct; extractor: "json-ld" }
  | { product: PageProduct; extractor: "micro-data" }
  | undefined {
 ...
 */
export function sightingFromPage(
  seller: Seller,
  url: string,
  html: string,
  checkedAt: string,
