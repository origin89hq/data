...
const text = (html: string): string =>
  decodeHTML(html.replace(/<br\s*\/?>/gi, " ").replace(/<[^>]*>/g, ""))
    .replace(/\u00a0/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
const ATTRIBUTE =
  /^(warranty|weight|voltage|capacity|current|power|dimensions?|size|colour|color|type|model|description|notes?|features?|price|part|sku|qty|quantity|series|温度|温度範囲)$/i;

/** A measurement is a value, so a header cell that is one belongs in the body: "5 Years", "40°C". */
const MEASUREMENT_CELL =
  /^\d+([.,]\d+)?\s*(years?|months?|days?|hours?|°?[cf]\b|kg|lbs?|mm|cm|m|in|v|a|w|ah|wh|kwh)\b/i;
