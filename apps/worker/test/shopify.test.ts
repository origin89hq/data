...
import { type ShopifyProduct, shopifyPageUrl, sightingsFromShopify } from "../src/shopify.ts";
...
test("a product with variants becomes one sighting per variant, each carrying its own sku and price", () => {
  const product: ShopifyProduct = {
    ...
    variants: [
      { title: "6V", sku: "S-550", price: "650.00", available: true },
      { title: "Pair", sku: "S-550x2", price: "1250.00", available: false },
    ],
  };
...
test("a default variant carries no variant label, and empty strings are absent, not empty", () => {
  const product: ShopifyProduct = {
    ...
    product_type: "",
    tags: "",
    variants: [{ title: "Default Title", sku: "", price: "9.99" }],
  };
  const [s] = sightingsFromShopify(seller, [product], "2026-09-09");
...
test("comma-separated tags are split and trimmed", () => {
  const [s] = sightingsFromShopify(
    seller,
    [{ id: 3, title: "T", handle: "t", tags: "a, b ,c" }],
    "2026-09-09",
  );
...
test("a sighting with a malformed price or date is refused by the schema", () => {
  const good = sightingsFromShopify(
    seller,
    [{ id: 4, title: "T", handle: "t", variants: [{ price: "1.00" }] }],
    "2026-09-09",
  )[0];
