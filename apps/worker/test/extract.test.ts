...
test("a sheet is read a window at a time, each window kept, and the reading written once all are in", async () => {
  ...
  const { env, asked, readObject } = world({ ...SORTED, [MARKDOWN]: SHEET }, reader());
...
test("a figure in a reading cites the page its value is printed on, and never the model's page", async () => {
  ...
  const unpaged = world({ ...SORTED, [MARKDOWN]: "| Weight | 42 kg |\n" }, answer);
  ...
  const first = world({ ...SORTED, [MARKDOWN]: paged }, answer);
...
test("an answer cut short is left for the queue, and the next delivery reads only the window missed", async () => {
  ...
  const { env, asked, read, readObject } = world({ ...SORTED, [MARKDOWN]: SHEET }, reader(broken));
...
test("a window Kimi's pace turns away waits its turn: what was read is kept, and the document goes back on the queue", async () => {
  ...
  const waited = world({ ...SORTED, [MARKDOWN]: SHEET }, reader());
...
test("a document turned away before reading anything waits longer each time, up to five minutes", async () => {
  const delayAfter = async (waits: number) => {
    const { env, sent, delays, pace, asked } = world({ ...SORTED, [MARKDOWN]: SHEET }, reader());
    pace.allow = () => false;
...
test("Kimi refusing for its own limit is waiting too, even on the last delivery, and never a failure", async () => {
  const limited = (call: number, input: TestAiInput) =>
    call === 2
      ...
      : reader()(call, input);
  const { env, asked, sent, read } = world({ ...SORTED, [MARKDOWN]: SHEET }, limited);
  await readDocument(message, env, LAST_ATTEMPT);
  ...
  const resumed = world(
    { ...SORTED, [MARKDOWN]: SHEET, [windowKey(1)]: `${JSON.stringify(read(windowKey(1)))}\n` },
    reader(),
  );
...
test("past its last wait a document asks no turn, and a refusal fails its window like any other error", async () => {
  const limited = (call: number, input: TestAiInput) =>
    call === 2
      ? new Error("3021: rate limiting: inference request per min rate reached")
      : reader()(call, input);
  const { env, pace, sent, readObject } = world({ ...SORTED, [MARKDOWN]: SHEET }, limited);
  pace.allow = () => false;
...
test("a window that fails on the last attempt is written down, and the reading still finishes", async () => {
  const { env, readObject } = world({ ...SORTED, [MARKDOWN]: SHEET }, reader(new Set([2])));
  await readDocument(message, env, LAST_ATTEMPT);
...
test("a window kept past this message's budget is not counted in its reading", async () => {
  ...
  const { env, asked, readObject } = world(
    { ...SORTED, [MARKDOWN]: SHEET, [windowKey(3)]: `${JSON.stringify(kept)}\n` },
    reader(),
  );
...
test("more windows kept than one listing returns are all found, so only the missing one is read", async () => {
  ...
  const objects: Record<string, string> = { ...SORTED, [MARKDOWN]: long };
...
test("only ratings of the maker's own products are kept: a family, a kit, another company's product, a setting, an instruction, a test and an example are not", async () => {
  const labelled = () => ({
    response: JSON.stringify({
      products: [
        {
          ...
          is: "product",
          specs: [
            { name: "Weight", value: "42", unit: "kg", is: "rating" },
            { name: "Float voltage", value: "13.8", unit: "V", is: "setting" },
            { name: "Wire size", value: "6", unit: "AWG", is: "instruction" },
            { name: "Test current", value: "25", unit: "A", is: "test" },
            { name: "Screen voltage", value: "12.6", unit: "V", is: "example" },
        ...
        },
        {
          model: "Optima 34M",
          is: "other-maker",
          specs: [{ name: "CCA", value: "800", is: "rating" }],
        },
        { model: "S-600", specs: [{ name: "Weight", value: "42", unit: "kg" }] },
      ],
    }),
  });
  const text =
    "| Weight | 42 kg | Capacity 428 Ah | Float 13.8 V | 6 AWG | 25 A | 12.6 V | CCA 800 |\n";
  const { env, readObject } = world({ ...SORTED, [MARKDOWN]: text }, labelled);
  await readDocument(message, env, 1);
  assert.deepEqual(
    readObject<Reading>(readingKey).products.map((p) => [p.model, p.specs.map((s) => s.name)]),
    [
      ["S-550", ["Weight", "Capacity"]],
      ["S-600", ["Weight"]],
    ],
    "a figure or product the model gave no label is kept rather than lost",
  );
  assert.equal(
    JSON.stringify(readObject<Reading>(readingKey)).includes('"is"'),
    false,
    "the labels are not written into the reading",
  );
});
...
test("a figure whose value the window does not print is left out of the reading", async () => {
  ...
  const { env, readObject } = world(
    { ...SORTED, [MARKDOWN]: "| S-550 | Weight | 42 kg |\n" },
    answer,
  );
  await readDocument(message, env, 1);
  assert.deepEqual(
...
test("a window whose answer has a note or a second object after its JSON is read, not failed", async () => {
  ...
  const { env, readObject } = world({ ...SORTED, [MARKDOWN]: "| Weight | 42 kg |\n" }, () => ({
...
test("an answer in a code fence is read, and prose in place of JSON leaves the window for the queue", async () => {
  ...
  const fenced = world({ ...SORTED, [MARKDOWN]: sheet }, () => ({
  ...
  const prose = world({ ...SORTED, [MARKDOWN]: sheet }, () => ({
    response: "The S-550 weighs 42 kg.",
  }));
...
test("a figure printed with an ASCII operator or a full-width tilde keeps its page when its name is reworded (#145)", async () => {
  ...
  const { env, readObject } = world({ ...SORTED, [MARKDOWN]: sheet }, () => ({
    response: JSON.stringify({
      products: [
        {
          model: "S-550",
...
test("the text reader sends a window without the symbols it garbles, and a figure keeps the symbol and page its document prints", async () => {
  ...
  const { env, asked, readObject } = world({ ...SORTED, [MARKDOWN]: sheet }, (_call, input) => {
