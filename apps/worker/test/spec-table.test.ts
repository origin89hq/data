...
test("a column per model becomes a product, with the figure names the maker wrote", () => {
  ...
  assert.deepEqual(
    ...
    ["MPPT 75/10", "MPPT 75/15", "MPPT 100/20"],
  );
  const small = products[0];
  assert.deepEqual(
    small.specs.find((s) => s.name === "Maximum battery current"),
    { name: "Maximum battery current", value: "10", unit: "A" },
  );
});
...
test("a dash is absence, not a figure, so a model without one carries nothing", () => {
  ...
  assert.equal(
    a.specs.some((s) => s.name === "Nominal PV power, 48V"),
    false,
  );
...
test("a table of footnotes is not a table of products", () => {
  assert.equal(
    parseSpecTables(page).some((p) => p.model === "Note" || p.model === "Meaning"),
    false,
  );
  assert.deepEqual(
    parseSpecTables(
      "<table><tr><th>Description</th><th>Notes</th></tr><tr><td>a</td><td>b</td></tr></table>",
    ),
    [],
  );
});
...
<tr><td>Colour</td><td>Blue</td><td>Blue</td></tr>
  ...
  assert.equal(
    products[0].specs.some((s) => s.name === "ENCLOSURE"),
    false,
  );
...
<tr><td>Maximum current</td><td>10A</td><td>20A</td></tr></table>`;
  assert.deepEqual(
    parseSpecTables(upright).map((p) => p.model),
    ["SL-10L-12V", "SL-20L-24V"],
  );
});
