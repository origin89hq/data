...
test("every pattern is a regex that compiles and is not double-escaped into matching nothing", () => {
  for (const s of sellers) {
    for (const [name, pattern] of [
      ...
      ["sitemapPattern", s.sitemapPattern],
    ] as const) {
      if (!pattern) continue;
      ...
      assert.doesNotMatch(
        pattern,
        /\\\\/,
        `${s.id}: ${name} has a doubled backslash, which matched nothing on NAZ and produced a crawl of zero urls`,
      );
    }
  }
});
...
test("a seller's product pattern matches a url from its own shop", () => {
  ...
  for (const [id, url] of Object.entries(samples)) {
    ...
    assert.match(
      url,
      new RegExp(seller.productPattern),
      `${id}: pattern does not match a real product url`,
    );
  }
});
...
test("a seller without a feed carries the sitemap details the page tier needs", () => {
  for (const s of sellers.filter((x) => !hasFeed(x) && x.platform === "bigcommerce")) {
    assert.ok(
      s.sitemapUrl,
      `${s.id}: BigCommerce does not serve /sitemap.xml, so a sitemapUrl is required`,
    );
  }
});
