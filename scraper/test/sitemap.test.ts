...
import { isIndex, locations, MAX_URLS, pickProducts, pickSitemaps, sample, sitemapUrl } from "../src/sitemap.ts";
import type { Seller } from "../../schema/sighting.ts";

const seller: Seller = { id: "s", name: "S", url: "https://s.example", country: "US", currency: "USD", platform: "bigcommerce" };
...
test("a seller's own sitemap location wins, since BigCommerce does not serve /sitemap.xml", () => {
  ...
  assert.equal(sitemapUrl({ ...seller, sitemapUrl: "https://s.example/xmlsitemap.php" }), "https://s.example/xmlsitemap.php");
});

test("only the index entries a seller names are opened, so a crawl does not walk the blog", () => {
  const all = ["https://s.example/x.php?type=pages", "https://s.example/x.php?type=products&page=1", "https://s.example/x.php?type=news"];
...
test("product urls are deduplicated, and a shop bigger than the cap says so instead of shrinking quietly", () => {
  ...
  assert.deepEqual(pickProducts({ ...seller, productPattern: "\\.html$" }, urls), { urls: ["https://s.example/a.html"], capped: false });
...
test("with no pattern every page is tried, because a page that publishes no product yields nothing anyway", () => {
  assert.deepEqual(pickProducts(seller, ["https://s.example/about"]), { urls: ["https://s.example/about"], capped: false });
});
