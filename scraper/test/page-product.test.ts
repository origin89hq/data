...
import { elementInner, extractProduct, fromJsonLd, fromMicrodata, modelOf, normalisePrice, sightingFromPage } from "../src/page-product.ts";
import { Sighting, type Seller } from "../../schema/sighting.ts";

const seller: Seller = { id: "naz", name: "NAZ", url: "https://naz.example", country: "US", currency: "USD", platform: "magento" };
...
test("a brand read from a nested Brand scope loses the theme's label, so it can be matched to a manufacturer", () => {
  ...
  assert.equal(fromMicrodata(bigcommerce.replace('<dd itemprop="brand" itemscope itemtype="http://schema.org/Brand"><span itemprop="name">IntegraRack</span></dd>', '<dd itemprop="brand">Brand : IntegraRack</dd>'))?.brand, "IntegraRack");
});
...
test("an element's extent is counted, so a div inside a div does not close it early", () => {
  ...
  assert.equal(elementInner('<meta itemprop="price" content="1">rest', 0, '<meta itemprop="price" content="1">', "meta"), "");
...
test("a page with no product data yields nothing rather than a sighting made of the page furniture", () => {
  ...
  assert.equal(fromJsonLd(`<script type="application/ld+json">{"@type":"Organization","name":"NAZ"}</script>`), undefined);
  assert.equal(sightingFromPage(seller, "https://naz.example/about.html", "<html></html>", "2026-09-09"), undefined);
});
...
test("a page sighting keys on the url path, because the page tier has no store id", () => {
  const s = sightingFromPage(seller, "https://naz.example/concorde-sunxtender-pvx-340t.html", magento, "2026-09-09");
...
test("an mpn that differs from the sku is kept, because it is a model number far more often", () => {
  const s = sightingFromPage(seller, "https://naz.example/x.html", magento.replace('"mpn":"PVX-340T"', '"mpn":"PVX340T-FT"'), "2026-09-09");
