...
export async function classifyRun(env: Env, seller: string, date: string): Promise<{ parts: number; sightings: number; alreadyAnswered: number }> {
  const manifest = await env.ARCHIVE.get(`sightings/${seller}/${date}/manifest.json`);
  if (!manifest) throw new Error(`sightings/${seller}/${date}: no manifest, the crawl did not finish`);
  const pages = (await manifest.json<{ pages: { page: number }[] }>()).pages.map((p) => p.page);

  const sightings: Sighting[] = [];
  for (const page of pages) {
    const object = await env.ARCHIVE.get(`sightings/${seller}/${date}/page-${String(page).padStart(4, "0")}.jsonl`);
  ...
  await env.ARCHIVE.put(`guesses/${seller}/${date}/${classifierKey()}/manifest.json`, JSON.stringify({ seller, checkedAt: date, by: classifierKey(), parts: parts.length, sightings: sightings.length, alreadyAnswered: sightings.length - fresh.length, pages: parts.map((p, i) => ({ page: i + 1, count: p.length })) }, null, 2), {
    httpMetadata: { contentType: "application/json" },
  });
  await sendAll(env.WORK, parts.map((batch, i): Work => ({ kind: "classify", seller, date, part: i + 1, sightings: batch })));
...
export async function convertRun(env: Env, manufacturer: string, date: string): Promise<{ documents: number }> {
  const manifest = await env.ARCHIVE.get(`documents/${manufacturer}/${date}/manifest.json`);
  if (!manifest) throw new Error(`documents/${manufacturer}/${date}: nothing approved to convert`);
  ...
  await env.ARCHIVE.put(`documents/${manufacturer}/${date}/converting.json`, JSON.stringify({ manufacturer, checkedAt: date, converter: CONVERTER, extractedBy: EXTRACTOR_ID, documents: unique }, null, 2), {
    httpMetadata: { contentType: "application/json" },
  });
  await sendAll(env.WORK, unique.map((d): Work => ({ kind: "convert", manufacturer, date, sha256: d.sha256, url: d.url, contentType: d.contentType })));
