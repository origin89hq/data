 ...
 */
export function sightingsFromShopify(seller: Seller, products: ShopifyProduct[], checkedAt: string): Sighting[] {
  const out: Sighting[] = [];
...
function tagsOf(tags: ShopifyProduct["tags"]): { tags?: string[] } {
  ...
  if (present(tags)) return { tags: tags.split(",").map((t) => t.trim()).filter(Boolean) };
  return {};
}
