...
export async function fetchMakers(): Promise<Maker[]> {
  ...
  return lines.map((line) => {
    ...
    return { id: value("id") ?? "", name: value("name") ?? "", website: value("website"), logo: value("logo"), logo_from: value("logo_from") };
  });
}

/** Rows for a table in the index, or zero when it is not published. */
export const rowsOf = (index: Index | undefined, table: string): number => index?.files[`${table}.parquet`]?.rows ?? 0;

export const count = (n: number): string => n.toLocaleString("en-US");
export const kb = (bytes: number): string => `${Math.round(bytes / 1024).toLocaleString("en-US")} KB`;
