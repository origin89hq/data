...
console.log("RECORDS");
console.log(`  ${records.manufacturers.length} manufacturers · ${records.models.length} models · ${specs} figures over ${withFigures} models`);
...
for (const b of inScope.slice(0, 5)) console.log(`      ${b.brand} — ${b.evidence.inScope} listings, ${b.evidence.models.slice(0, 2).join(", ")}`);
...
if (state) {
  const stale = state.sellers.filter((s) => s.classified && s.classified.written < s.classified.parts);
  console.log(`\nSELLERS (${state.sellers.length} crawled)`);
  console.log(`  ${state.sellers.reduce((n, s) => n + (s.sightings ?? 0), 0)} sightings in the last crawl of each`);
  if (stale.length) {
    console.log(`  ${stale.length} with a classification that did not finish:`);
    for (const s of stale.slice(0, 5)) console.log(`      ${s.seller}: ${s.classified!.written} of ${s.classified!.parts} parts`);
  }
