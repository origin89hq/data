...
const dir = args.includes("--dir")
  ? (args[args.indexOf("--dir") + 1] ?? "dist/logos")
  : "dist/logos";
...
for (const maker of records.manufacturers) {
  for (const width of maker.logo?.widths ?? [])
    expected.set(`${maker.id}-${width}.png`, logoKey(maker.id, width));
}
...
if (orphans.length)
  console.log(
    `${orphans.length} files on disk that no manufacturer record claims, skipped: ${orphans.slice(0, 5).join(", ")}`,
  );
if (missing.length)
  console.log(
    `${missing.length} records name a width with no file: ${missing.slice(0, 5).join(", ")}`,
  );
...
console.log(
  `${put} logo files ${dryRun ? "would be uploaded" : "uploaded"} for ${records.manufacturers.filter((m) => m.logo).length} manufacturers`,
);
if (!dryRun)
  console.log(`check one: curl -sI https://data.origin89.com/${logoKey("victron-energy", 128)}`);
