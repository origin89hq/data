 ...
 */
const saysSomething = (name: string): boolean => /[A-Za-z]{4}/.test(name) || /(^|\s)[A-Za-z]{3}(\s|$)/.test(name);
...
if (unevidenced) console.log(`${unevidenced} out-of-scope answers refused: a bare part number with no figures is no evidence for a claim about what a product is`);
