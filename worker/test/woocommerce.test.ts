...
import { minorToDecimal, sightingsFromWoo, wooPageUrl, type WooProduct } from "../src/woocommerce.ts";
import { Sighting, type Seller } from "../../schema/sighting.ts";

const seller: Seller = { id: "woo", name: "Woo", url: "https://woo.example", country: "CA", currency: "CAD", platform: "woocommerce" };

const victron: WooProduct = {
  id: 61844, name: "Victron Lynx Power In (M10)", slug: "victron-lynx-power-in-m10", permalink: "https://woo.example/product/victron-lynx-power-in-m10/", sku: "VIC-104-215", type: "simple",
  prices: { price: "19610", currency_code: "CAD", currency_minor_unit: 2 }, brands: [], categories: [{ name: "Accessories" }, { name: "Busbars" }], tags: [{ name: "LYN020102010" }],
  attributes: [{ name: "Brand", taxonomy: "pa_brand", terms: [{ name: "Victron" }] }, { name: "Mnfr. model", taxonomy: "pa_mnfr-model", terms: [{ name: "LYN020102010" }] }], is_in_stock: true,
};
...
test("a brands array wins over the brand attribute, since it is the store's own field", () => {
  const [s] = sightingsFromWoo(seller, [{ ...victron, brands: [{ name: "Victron Energy" }] }], "2026-09-09");
...
test("a product with no price, brand, sku or stock flag still yields a sighting with those absent", () => {
  const [s] = sightingsFromWoo(seller, [{ id: 1, name: "Bare", slug: "bare", permalink: "https://woo.example/product/bare/" }], "2026-09-09");
...
test("a non-integer price string is refused rather than mis-scaled", () => {
  const [s] = sightingsFromWoo(seller, [{ ...victron, prices: { price: "196.10", currency_minor_unit: 2 } }], "2026-09-09");
  assert.equal(s.price, undefined);
});
...
test("an empty page yields nothing", () => {
  ...
  assert.equal(wooPageUrl(seller, 2), "https://woo.example/wp-json/wc/store/v1/products?per_page=100&page=2");
});
