...
import { LONG_EDGE, png, pngDataUrl, renderPage, type Drawn } from "../src/render.ts";
import { BLACK_BOX, GREY_SCAN, pdfium, tinyPdf } from "./pdf.ts";

/** The pixel at (x, y), as [r, g, b]. */
const at = (drawn: Drawn, x: number, y: number): number[] => [...drawn.rgb.subarray((y * drawn.width + x) * 3, (y * drawn.width + x) * 3 + 3)];
...
function decode(bytes: Uint8Array): { width: number; height: number; rgb: Uint8Array } {
  assert.deepEqual([...bytes.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10], "a PNG starts with its signature");
  ...
  while (offset < bytes.length) {
    ...
    assert.equal(view.getUint32(offset + 8 + length), crc32(bytes.subarray(offset + 4, offset + 8 + length)), `the ${type} checksum holds`);
    if (type === "IHDR") {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < row; x += 1) rgb[y * row + x] = (raw[y * (row + 1) + 1 + x] + (y > 0 ? rgb[(y - 1) * row + x] : 0)) & 0xff;
  }
...
test("a placed image is drawn the way a scanned page is, grey where the scan is grey", async () => {
  ...
  assert.deepEqual(at(drawn, 300, 104), [255, 255, 255], "the half the image does not cover stays paper");
});
...
test("a page is drawn with its long edge capped, and never finer than 150 dpi", async () => {
  ...
  assert.deepEqual([poster.width, poster.height], [LONG_EDGE, LONG_EDGE / 2], "a poster is shrunk to fit");
...
test("pages are counted from one, and the second page is the second page", async () => {
  ...
  assert.deepEqual(at(renderPage(await pdfium(), tinyPdf(["", BLACK_BOX]), 1), 208, 104), [255, 255, 255], "page one is blank");
});
...
test("a page that is not there, or a file that is not a PDF, is refused with the reason", async () => {
  ...
  assert.throws(() => renderPage(library, new TextEncoder().encode("PK a zip of logos"), 1), /not a PDF, or a damaged one/);
...
test("a drawn page survives the round trip through PNG, and a one-pixel picture is still a picture", async () => {
  const drawn = renderPage(await pdfium(), tinyPdf([BLACK_BOX]), 1);
  assert.deepEqual([...decode(await png(drawn.width, drawn.height, drawn.rgb)).rgb], [...drawn.rgb]);
...
test("the data URL carries the PNG whole, even past the size a spread can take", () => {
  ...
  assert.deepEqual([...Buffer.from(url.slice("data:image/png;base64,".length), "base64")], [...bytes]);
