...
export function tinyPdf(pages: string[], width = 200, height = 100): Uint8Array {
  ...
  for (const [i, content] of pages.entries()) {
    objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${width} ${height}] /Contents ${4 + i * 2} 0 R /Resources << >> >>`);
...
export function pdfium(): Promise<Pdfium> {
  opened ??= openPdfium(new WebAssembly.Module(readFileSync(fileURLToPath(import.meta.resolve("@embedpdf/pdfium/pdfium.wasm")))));
  return opened;
}
