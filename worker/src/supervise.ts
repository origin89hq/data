...
export async function supervise(env: Env, today: string): Promise<SupervisionReport> {
  ...
  for (const seller of await sellerStates(env.ARCHIVE)) {
    ...
    if (!seller.classified) {
      ...
      report.started.push({ what: "classify", entity: seller.seller, detail: `${parts} batches, ${alreadyAnswered} listings already answered` });
      continue;
    }
    ...
    if (missing > 0) report.concerns.push(`${seller.seller} classified ${seller.classified.written} of ${seller.classified.parts} parts on ${seller.date}`);
  }
  ...
  for (const maker of await makerStates(env.ARCHIVE)) {
    ...
    if (pages.has(maker.maker) && !maker.read) {
      ...
      if (sent > 0) report.started.push({ what: "spec-pages", entity: maker.maker, detail: `${sent} pages the maker publishes` });
    }

    if (maker.waitingOn.startsWith("somebody to approve")) {
      report.blocked.push({ entity: maker.maker, waitingOn: `approval for ${maker.offered ?? 0} documents` });
      continue;
    }
  ...
  console.log(JSON.stringify({ message: "supervision finished", started: report.started.length, blocked: report.blocked.length, concerns: report.concerns.length }));
  return report;
}
