...
import { png, pngDataUrl, renderPage, type Pdfium } from "./render.ts";
...
export async function seeDocument(message: VisionDocument, env: Env): Promise<void> {
  ...
  if (source.size > MAX_RENDER_BYTES) {
    ...
    await env.ARCHIVE.put(reading, `${JSON.stringify({ sha256: message.sha256, url: message.url, products: [], pages: 0, failed: 0, refused, extractedBy: VISION_EXTRACTOR_ID })}\n`, AS_JSON);
    return;
  }
  const pages = Math.min(layer.pages, MAX_PAGES);
  if (layer.pages > MAX_PAGES) console.log(JSON.stringify({ message: "reading the first pages only", sha256: message.sha256, pages: layer.pages, reading: pages }));
  const { run, manufacturer, date, sha256, url } = message;
  await sendAll(env.WORK, Array.from({ length: pages }, (_, i): Work => ({ kind: "vision-page", run, manufacturer, date, sha256, url, page: i + 1, pages })));
}

/** One page: drawn, written down and read once, then put together with the others if it was the last to land. */
export async function seePage(message: VisionPage, env: Env, attempt: number, library: () => Promise<Pdfium>): Promise<void> {
  ...
  if (!(await env.ARCHIVE.head(key))) await env.ARCHIVE.put(key, `${JSON.stringify(await readPage(message, env, attempt, library))}\n`, AS_JSON);
  await gatherPages(message, env);
}

/** Draw one page, write down what it says, and read the figures out of what was written. */
async function readPage(message: VisionPage, env: Env, attempt: number, library: () => Promise<Pdfium>): Promise<SeenPage> {
  const { page } = message;
  ...
  try {
    const response = await env.AI.run(VISION_MODEL, {
      messages: [
        ...
        { role: "user", content: [{ type: "text", text: "Transcribe this page." }, { type: "image_url", image_url: { url: image } }] },
      ],
      response_format: { type: "json_schema", json_schema: { name: "page", schema: TRANSCRIPT_SCHEMA, strict: false } },
      max_tokens: 6000,
      ...quietly,
    } as never);
  ...
  try {
    const response = await env.AI.run(VISION_MODEL, {
      ...
      response_format: { type: "json_schema", json_schema: { name: "figures", schema: VISION_RESPONSE_SCHEMA, strict: false } },
      max_tokens: 4096,
      ...quietly,
    } as never);
...
async function gatherPages(message: VisionPage, env: Env): Promise<void> {
  const listed = await env.ARCHIVE.list({ prefix: partKey.pages(message.sha256, READER), limit: 1000 });
  const pages = (await Promise.all(listed.objects.map(async (object) => (await env.ARCHIVE.get(object.key))?.json<SeenPage>())))
  ...
  await env.ARCHIVE.put(transcript, transcriptDocument(name, pages), { httpMetadata: { contentType: "text/markdown" } });
  const reading = { sha256: message.sha256, url: message.url, products: mergeReports(pages.flatMap((p) => p.products)), pages: pages.length, failed: pages.filter((p) => p.failed).length, transcript, extractedBy: VISION_EXTRACTOR_ID };
  await env.ARCHIVE.put(partKey.reading(message.sha256, READER), `${JSON.stringify(reading)}\n`, AS_JSON);
}
