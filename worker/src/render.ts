...
const LOAD_ERRORS: Record<number, string> = { 1: "an unknown error", 2: "a file it could not open", 3: "not a PDF, or a damaged one", 4: "a password", 5: "a security handler it does not support", 6: "a missing page" };
  ...
  const pdfium = await Promise.race([
    init({
      instantiateWasm: (imports: WebAssembly.Imports, receive: (instance: WebAssembly.Instance, module: WebAssembly.Module) => void) => {
        WebAssembly.instantiate(module, imports).then((instance) => receive(instance, module), refuse);
        return {};
      },
 ...
 */
export function renderPage(pdfium: Pdfium, bytes: Uint8Array, page: number, longEdge = LONG_EDGE): Drawn {
  ...
  try {
    ...
    if (!document) throw new Error(`PDFium could not open it: ${LOAD_ERRORS[pdfium.FPDF_GetLastError()] ?? "an unknown error"}`);
    try {
...
const heap = (pdfium: Pdfium): Uint8Array => (pdfium.pdfium as unknown as { HEAPU8: Uint8Array }).HEAPU8;
...
export async function png(width: number, height: number, rgb: Uint8Array): Promise<Uint8Array> {
  if (rgb.length !== width * height * 3) throw new Error(`${rgb.length} bytes are not a ${width}×${height} RGB picture`);
  const row = width * 3;
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    const from = y * row;
    filtered[at] = 2;
    for (let x = 0; x < row; x += 1) filtered[at + 1 + x] = (rgb[from + x] - (y > 0 ? rgb[from - row + x] : 0)) & 0xff;
  }
